...
do id = 0, p%glb%threads-1
    ...
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        ...
        p%of(id)%loc%nvel%x%now(i,j,k) = -dsin(pi*x)**2 * dsin(2.0*pi*y) * ct
        p%of(id)%loc%nvel%y%now(i,j,k) = dsin(pi*y)**2 * dsin(2.0*pi*x) * ct

        p%of(id)%loc%vel%x%now(i,j,k) = -dsin(pi*xx)**2 * dsin(2.0*pi*y) * ct
        p%of(id)%loc%vel%y%now(i,j,k) = dsin(pi*yy)**2 * dsin(2.0*pi*x) * ct
...
subroutine rotating_disk_velocity()
use all 
!$ use omp_lib
implicit none
integer :: i,j,k,id
real(8) :: x, y, z, xx, yy, zz, pi, omega

pi = dacos(-1.0_8)
omega = 2.0 * pi / p%glb%t2s
...
do id = 0, p%glb%threads-1
    ...
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        ...
        p%of(id)%loc%nvel%x%now(i,j,k) = -omega*(y-0.5)
        p%of(id)%loc%nvel%y%now(i,j,k) = omega*(x-0.5)

        p%of(id)%loc%vel%x%now(i,j,k) = -omega*(y-0.5)
        p%of(id)%loc%vel%y%now(i,j,k) = omega*(x-0.5)
