...
do id = 0, p%glb%threads-1
    ...
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        ...
        if( p%of(id)%loc%sign%tmp(i,j,k) >= 0.0_8 )then
            ...
            p%of(id)%loc%grad%now(i,j,k) = dsqrt( MAX(upm,ump)**2.0d0 + MAX(vpm,vmp)**2.0d0  )
        else 
            ! p%of(id)%loc%grad%now(i,j,k) = dsqrt( MAX(upp,umm)**2.0d0 + MAX(vpp,vmm)**2.0d0 + MAX(wpp,wmm)**2.0d0 )
            p%of(id)%loc%grad%now(i,j,k) = dsqrt( MAX(upp,umm)**2.0d0 + MAX(vpp,vmm)**2.0d0 )
        end if
...
do id = 0, p%glb%threads-1
    ...
    do i = p%of(id)%loc%is, p%of(id)%loc%ie
        ...
        a = 17.0d0*p%of(id)%loc%tdata%x%s2(i,j,k)
        b = 17.0d0*p%of(id)%loc%tdata%x%s3(i,j,k)
