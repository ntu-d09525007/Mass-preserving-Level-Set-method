...
open(unit=456, file="./Vortex_Results.txt", action='write', position='append')

if( p%glb%method == 1)then
    write(456, *)"Method: LS"
else if ( p%glb%method == 2)then
    write(456, *)"Method: MPLS"
else    
    write(456,*)"Method: CLSVOF"
endif

write(456,*)"Density Ratio:", p%glb%rho_12
...
if(p%glb%method == 3)then
    ...
    write(456,'(A10, 2ES15.4)')"LS Vol:", p%glb%loss_vol_avg / p%glb%iter, p%glb%loss_vol_max
    write(456,'(A10, 2ES15.4)')"LS Int:", error / p%glb%node_x / p%glb%node_y / p%glb%node_z
endif
